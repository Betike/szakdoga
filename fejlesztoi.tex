...
\begin{itemize}
    ...
    \item \texttt{premier\_league\_match\_outcomes\_2017-2018\_to\_2024-2025.csv} - Mérkőzéseredmények
\end{itemize}

A mérkőzéseredmények dataset tartalmazza az alábbi mezőket:
\begin{itemize}
    \item \texttt{Season} - Bajnoki szezon (pl. "2023-2024")
    \item \texttt{Date} - A mérkőzés dátuma
    \item \texttt{HomeTeam} - Hazai csapat neve
    \item \texttt{AwayTeam} - Vendég csapat neve
    \item \texttt{HomeGoals} - Hazai csapat által szerzett gólok száma
    \item \texttt{AwayGoals} - Vendég csapat által szerzett gólok száma
    \item \texttt{Result} - Mérkőzés eredménye: "H" (hazai győzelem), "D" (döntetlen), "A" (vendég győzelem)
    \item \texttt{Venue} - A mérkőzés helyszíne
    \item \texttt{HomeXG}, \texttt{AwayXG} - xG (Expected Goals) értékek, ahol elérhetők
\end{itemize}
...
A \texttt{scraping.py} szkript felelős az adatgyűjtésért a FBref.com oldalról. Két fő funkciót tartalmaz:

\begin{itemize}
    \item \texttt{scrape\_premier\_league\_detailed\_stats(season)} - Egy adott szezon részletes statisztikáinak gyűjtése
    \item \texttt{scrape\_multiple\_seasons(seasons)} - Több szezon adatainak összegyűjtése és kombinálása
    \item \texttt{scrape\_premier\_league\_match\_outcomes(seasons)} - Mérkőzéseredmények gyűjtése
\end{itemize}

A script a ScraperAPI-t használja a lekérések kezelésére az oldal rate limiting elkerülése érdekében. Ennek a használatához saját ScraperAPI token igénylése, majd annak a beillesztése a "*token" helyekre a kódban. 
...
Az adatelőkészítést két fő szkript végzi:

\texttt{create\_team\_performance\_dataset.py} - A különböző statisztikai adatokat tartalmazó CSV fájlokat egyesíti egy \texttt{team\_performance.csv} fájlba, amely tartalmazza az összes releváns jellemzőt minden csapathoz minden szezonban.

\texttt{prepare\_model\_data.py} - A csapatteljesítmény-adatokat és a mérkőzéseredményeket egyesíti egy olyan formátumba, amely alkalmas a gépi tanulási modellek betanítására. Ez a szkript:
\begin{itemize}
...
A train modulban több különböző gépi tanulási modell van implementálva:
...
\begin{itemize}
    ...
    \item Egyéni \texttt{MatchPredictionNN} osztály amely a következő elemeket tartalmazza:
    \begin{itemize}
...
\subsubsection{Központi előrejelzési interfész}
...
\begin{itemize}
    ...
    \item A megfelelő előrejelzési szkript futtatása
...
\begin{verbatim}
python predict/run_prediction.py --model "ensemble" --home "Liverpool"
--away "Manchester City" --json
\end{verbatim}

\subsection{Compare modul}

A \texttt{compare\_models.py} szkript a különböző modellek teljesítményét hasonlítja össze a test dataseten:
...
\subsubsection{Projektstruktúra}

A frontend alkalmazás struktúrája a Next.js konvenciókat követ:
...
\begin{itemize}
    ...
    \item Interaktív leírások és magyarázatok a metrikákhoz
\end{itemize}
...
\begin{itemize}
    ...
    \item Node.js child\_process modul használata a Python scriptek futtatásához
...
A frontend alkalmazás adatfolyama a következő:
\begin{enumerate}
    ...
    \item Az API végpont futtatja a megfelelő Python szkriptet
    \item A Python szkript visszaadja az előrejelzési eredményeket
    \item Az eredmények megjelennek a felhasználói felületen vizuális formában
\end{enumerate}

\subsubsection{Dependency Management}

A frontend függőségeit a \texttt{package.json} fájl tartalmazza, a főbb függőségek:
\begin{itemize}
    \item next - Next.js keretrendszer
    \item react, react-dom - React könyvtárak
    \item tailwindcss - Utility-first CSS keretrendszer
    \item @hookform/resolvers - Form validációs könyvtár
    \item zod - Adatvalidációs könyvtár
    \item typescript - Típusos JavaScript
\end{itemize}
...
\textbf{Leírás:} Előrejelzést készít egy adott mérkőzésre a specifikált modell alapján.
...
\begin{verbatim}
{
  ...
  "away_team": "Liverpool",
  "model": "ensemble"  // vagy "random_forest", "xgboost", "neural_network"
}
\end{verbatim}
...
\begin{verbatim}
{
  "prediction": "H",  // "H" (hazai győzelem), "D" (döntetlen), "A" (vendég győzelem)
  "probabilities": {
    "home_win": 0.65,
    "draw": 0.20,
    "away_win": 0.15
...
\section{Adatfolyam és modellezési pipeline}

A teljes adatfolyamat a nyers adatok gyűjtésétől a végső előrejelzésig az alábbi ábrán látható:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/rendszer_adatfolyam.png}
    \caption{A Premier League Predictor adatfolyama}
    \label{fig:data-flow}
\end{figure}
...
\begin{itemize}
    ...
    \item A fájlnevek snake\_case konvenciót követnek
    \item A Python kód a PEP 8 stílusútmutatásokat követi
    \item A TypeScript kód az ESLint standardokat követi
    \item A modelleket a \texttt{models/} mappában tároljuk
    \item Az eredményeket és vizualizációkat a \texttt{visualisations/} mappában tároljuk
\end{itemize}

\section{Hibaelhárítás és naplózás}

\subsection{Gyakori hibák és megoldásaik}

\begin{enumerate}
    \item \textbf{Adatgyűjtési hibák:} Ha a scraping folyamat hibákkal zárul, ellenőrizze az API kulcs érvényességét és a célwebhely elérhetőségét. A scraping.py parancssorból való futtatása részletesebb hibaüzeneteket ad.
    \item \textbf{Modell betöltési hibák:} Ellenőrizze, hogy a modelleket betanították-e és a megfelelő helyeken tárolódnak.
    \item \textbf{Függőségi problémák:} Futtassa a \texttt{pip install -r requirements.txt} parancsot a szükséges Python csomagok telepítéséhez.
\end{enumerate}

\subsection{Naplózás}

A rendszer részletes naplózást alkalmaz a hibák és figyelmeztetések követésére. A naplófájlok a következő formátumot követik:
...
Az egységtesztek a \texttt{tests/} mappában találhatók, és a \texttt{pytest} keretrendszert használják:

\begin{verbatim}
python -m pytest tests/
\end{verbatim}

\subsection{Teljesítménytesztek}

A modellteljesítmény tesztelését a \texttt{compare\_models.py} szkript végzi, amely részletes metrikákat és összehasonlításokat generál:
...
\begin{enumerate}
    \item Hozzon létre egy új szkriptet a \texttt{train/} mappában, követve a meglévő minták struktúráját
    \item Implementálja a modell betanítását és mentését
    \item Hozzon létre egy megfelelő predikciós szkriptet a \texttt{predict/} mappában
    \item Frissítse a \texttt{compare\_models.py} szkriptet az új modell teljesítményének kiértékelésére
    \item Frissítse a \texttt{predict\_ensemble.py} szkriptet, ha az új modellt be szeretné építeni az ensemble megközelítésbe
...
\begin{enumerate}
    \item Bővítse a \texttt{scraping.py} szkriptet az új forrás kezelésére
    \item Frissítse az adatelőkészítési szkripteket az új adatok integrálásához
